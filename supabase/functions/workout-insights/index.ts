  ...
  try {
    ...
    workouts.forEach(w => {
      ...
      w.exercises.forEach(ex => {
        ...
        const maxWt = Math.max(...ex.sets.map(s => parseFloat(s.weight) || 0).filter(v => v > 0));
        if (maxWt <= 0) return;
    ...
    if (ANTHROPIC_API_KEY) {
      ...
      if (insights) {
        // client.from(...).upsert(...) returns a thenable, not a full Promise —
        // it has no .catch(), so chaining .catch() directly threw before the
        // request even fired (BUG-GAINS-UPSERT-CATCH). await inside try/catch instead.
        try {
          ...
          // silent fail — insights still returned in this response, cache write retried next request
        }
      }
    }
...
async function generateInsights(data, apiKey) {
  const prompt = `You are Jason's fitness coach. Be direct and specific.
